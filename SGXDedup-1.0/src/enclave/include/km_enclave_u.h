...
sgx_status_t sgx_ra_get_msg3_trusted(sgx_enclave_id_t eid, sgx_status_t* retval, sgx_ra_context_t context, uint32_t quote_size, sgx_report_t* qe_report, sgx_ra_msg3_t* p_msg3, uint32_t msg3_size);
 sgx_status_t ecall_processSignedHash(sgx_enclave_id_t eid, sgx_status_t* retval, uint8_t* hmac,uint8_t* chunkHashList, int hashSize, uint8_t* keyList, uint8_t* tagList);
 sgx_status_t ecall_verifyChunkSig(sgx_enclave_id_t eid, sgx_status_t* retval, uint8_t* chunkHmac, uint8_t* chunk, uint32_t size);
#ifdef __cplusplus
}
#endif /* __cplusplus */
