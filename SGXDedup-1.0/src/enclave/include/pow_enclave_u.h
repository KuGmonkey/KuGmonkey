...
sgx_status_t ecall_setCurrentSessionKey(sgx_enclave_id_t eid, sgx_status_t* retval, unsigned char* currentSessionKey, unsigned char* hmacKey);
...
sgx_status_t ecall_keyExchangeEncrypt(sgx_enclave_id_t eid, sgx_status_t* retval, uint8_t* chunkData, uint32_t srcLen, uint32_t number, uint8_t* hmac, uint8_t* chunkHashList);
sgx_status_t ecall_sendEncrypt(sgx_enclave_id_t eid, sgx_status_t* retval, uint32_t batchNumber, uint32_t totalNumber, uint8_t* cipherKey, uint8_t* plainkey, uint8_t* serverHmac, uint8_t* upload, uint8_t* cipherBlock, uint8_t* sigList);
#ifdef __cplusplus
}
#endif /* __cplusplus */
