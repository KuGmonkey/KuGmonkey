...
bool keyServer::generateKeywithPOW(uint8_t* hmac,uint8_t* chunkHashList, int hashSize, uint8_t* keyList, uint8_t* tagList)
{
    return client->generateKey(hmac,chunkHashList, hashSize, keyList, tagList);
}
...
bool keyServer::verifyChunk(uint8_t* chunk, uint32_t size, uint8_t* chunkHmac)
{
    return client->verifyChunk(chunk,size,chunkHmac);
}
